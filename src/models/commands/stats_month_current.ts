...
export const stats_month_current = async (
  bot: TelegramBot,
  ctx: Message,
  connection: any,
  ...
  try {
    console.log(`/stats_month_current - ${ctx.chat.title}`);
    ...
    const statsMessage: string = getStatsMessageByDay({
      ...
      date: Dates.CURRENT_MONTH,
      dailyData,
    });

    await bot.sendMessage(ctx.chat.id, statsMessage);
  } catch (err: any) {
    console.log("Stats Current Month Error::", err.message);
  }
};
